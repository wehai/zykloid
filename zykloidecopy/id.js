...
/*     app.get('/content', async function(req, res){
      ...
      const content = await gallerydb.findOneAsync({})
      console.log(content)
      const z1 = content.zug1.submissions
      const z2 = content.zug2.submissions
      const z3 = content.zug3.submissions
      const z4 = content.zug4.submissions
      const z5 = content.zug5.submissions
      const z6 = content.zug6.submissions
console.log(z1 + z2)
...
let scont;
let simg;
/*     // await gallerydb.updateAsync({_id: content._id},{$set: {'zug1': [] }})
     abfolge1.forEach(async function(element) {
      ...
      scont = {img: cimg}}
      console.log(scont)
      //await gallerydb.updateAsync({_id: content._id},{$push: {'zug1': scont }})
       })
   
  
   //const updatedContent = await gallerydb.findAsync({'posted':"false"})
  
   //res.json(updatedContent)
   //await gallerydb.updateAsync({_id: content._id},{$set: {'posted':"true"}},{ multi: true })
    //});
